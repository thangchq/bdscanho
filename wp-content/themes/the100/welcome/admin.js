...
jQuery(document).ready(function($) {
	...
	$(".install").on('click', function (e) {
    	...
    	var ajaxurl = the100WelcomeObject.ajaxurl;
    	...
    	var plNonce = newPlhref[newPlhref.length-1];
    	if(plNonce==''){
    		var plNonce = the100WelcomeObject.admin_nonce;
    	}
		...
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'the100_plugin_installer',
				plugin: plugin,
			...
			success: function(response) {
		   		...
		   		if(response == 'success'){
				   		...
				   		el.html(the100WelcomeObject.installed_btn);
	...
	$('.install-offline').on('click', function (e) {
		...
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'the100_plugin_offline_installer',
	...
	$(".activate").on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'the100_plugin_activation',
	   			plugin: plugin,
	   			nonce: the100WelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
	   		success: function(response) {
		   		if(response){
	...
	$('.activate-offline').on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'the100_plugin_offline_activation',
	   			plugin: plugin,
	   			nonce: the100WelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
