...
$options = cryout_get_option( array( 'septera_excerptarchive', 'septera_excerptsticky', 'septera_excerpthome' ) );
...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'hentry' ); cryout_schema_microdata( 'blogpost' ); ?>>
	...
	<div class="article-inner">
		...
		if ( $options['septera_excerptarchive'] == "full" ) { $mode = 'content'; }
		if ( is_sticky() && $options['septera_excerptsticky'] == "full" ) { $mode = 'content'; }
		if ( $options['septera_excerpthome'] == "full" && ! is_archive() && ! is_search() ) { $mode = 'content'; }
