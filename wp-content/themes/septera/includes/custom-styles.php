...
function septera_body_classes( $classes ) {
	...
	switch ( $options['septera_articleanimation'] ) {
...
#breadcrumbs-container 						{ border-bottom-color: <?php echo esc_html( cryout_hexdiff( $septera_contentbackground, 17 ) ); ?>;
											  background-color: <?php echo esc_html( cryout_hexdiff( $septera_contentbackground, 12 ) ) ?>; }
...
<?php }
	...
	return apply_filters( 'septera_custom_styles', preg_replace('/((background-)?color:\s*?)[;}]/i', '', ob_get_clean() ) );
